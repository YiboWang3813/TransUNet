...
from networks.main_net import MainNet, MainNetWithoutTF, MainNetWithSingleTF
...
class MainModel(BaseModel):
    def __init__(self, opt):
        ...
        self.net = MainNet(opt.enc_nc_main, opt.dec_nc_main, opt.patch_size, opt.num_heads) 
        # self.net = MainNetWithoutTF(opt.enc_nc_main, opt.dec_nc_main, opt.patch_size, opt.num_heads) 
        # self.net = MainNetWithSingleTF(opt.enc_nc_main, opt.dec_nc_main, opt.patch_size, opt.num_heads) 
        self.net.apply(weights_init_normal)

        if opt.is_train == 0:
            self.net.to(self.device)
            self.load_network('main', opt.which_epoch, self.net)
